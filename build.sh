...
# Clean early if needed (optional)
...
# Clean source tree after patches
log "Cleaning source tree with mrproper..."
make ARCH=arm64 mrproper
...
cd $KSRC
mkdir -p out
BUILD_CONFIG=common/build.config.gki O=out make gki_defconfig || make defconfig || exit 1
...
../scripts/config --enable CONFIG_KSU_SUSFS 2>/dev/null || log "SuSFS not present"
../scripts/config --enable CONFIG_KSU_SUSFS_AUTO_ADD 2>/dev/null || log "SuSFS auto add not present"
...
# Post-compiling (AnyKernel, zip, etc.)
cd $workdir
